...
func main() {
	fx.New(
		fx.Provide(
			...
			fx.Annotate(imageregistry.NewCraneContainerImageRegistry, fx.As(new(imageregistry.ImageRegistry))),
			...
			// delete steps
			initfx.AsDeleteStep(deletion.NewServiceAccountRemoverStep),
			initfx.AsDeleteStep(deletion.NewUnregisterDoguVersionStep),
			initfx.AsDeleteStep(deletion.NewDeleteOutOfHealthConfigMapStep),
			initfx.AsDeleteStep(deletion.NewRemoveDoguConfigStep, fx.ParamTags(`name:"normalDoguConfig"`)),
			initfx.AsDeleteStep(deletion.NewRemoveSensitiveDoguConfigStep, fx.ParamTags(`name:"sensitiveDoguConfig"`)),
			initfx.AsDeleteStep(deletion.NewRemoveFinalizerStep),

			// install or change steps
			initfx.AsInstallOrChangeStep(install.NewConditionsStep),
			initfx.AsInstallOrChangeStep(install.NewHealthCheckStep),
			initfx.AsInstallOrChangeStep(install.NewValidationStep),
			initfx.AsInstallOrChangeStep(install.NewFinalizerExistsStep),
			// Dogu config steps
			initfx.AsInstallOrChangeStep(install.NewCreateConfigStep, fx.ParamTags(`name:"normalDoguConfig"`)),
			initfx.AsInstallOrChangeStep(install.NewOwnerReferenceStep, fx.ParamTags(`name:"normalDoguConfig"`)),
			// Sensitive dogu config steps
			initfx.AsInstallOrChangeStep(install.NewCreateConfigStep, fx.ParamTags(`name:"sensitiveDoguConfig"`)),
			initfx.AsInstallOrChangeStep(install.NewOwnerReferenceStep, fx.ParamTags(`name:"sensitiveDoguConfig"`)),
			// Create local dogu descriptor and set owner reference
			initfx.AsInstallOrChangeStep(install.NewRegisterDoguVersionStep),
			initfx.AsInstallOrChangeStep(install.NewOwnerReferenceStep, fx.ParamTags(`name:"localDoguDescriptorRepository"`)),
			initfx.AsInstallOrChangeStep(install.NewServiceAccountStep),
			initfx.AsInstallOrChangeStep(install.NewServiceStep),
			initfx.AsInstallOrChangeStep(install.NewExecPodCreateStep),
			initfx.AsInstallOrChangeStep(install.NewCustomK8sResourceStep),
			initfx.AsInstallOrChangeStep(install.NewVolumeGeneratorStep),
			initfx.AsInstallOrChangeStep(install.NewNetworkPoliciesStep),
			initfx.AsInstallOrChangeStep(install.NewDeploymentStep),
			initfx.AsInstallOrChangeStep(postinstall.NewReplicasStep),
			initfx.AsInstallOrChangeStep(postinstall.NewVolumeExpanderStep),
			initfx.AsInstallOrChangeStep(postinstall.NewAdditionalIngressAnnotationsStep),
			initfx.AsInstallOrChangeStep(postinstall.NewSecurityContextStep),
			initfx.AsInstallOrChangeStep(postinstall.NewExportModeStep),
			initfx.AsInstallOrChangeStep(postinstall.NewSupportModeStep),
			initfx.AsInstallOrChangeStep(postinstall.NewAdditionalMountsStep),
			initfx.AsInstallOrChangeStep(postinstall.NewRestartDoguStep, fx.ParamTags("", "", `name:"normalDoguConfig"`, `name:"sensitiveDoguConfig"`, "")),
			initfx.AsInstallOrChangeStep(upgradeSteps.NewEqualDoguDescriptorsStep),
			initfx.AsInstallOrChangeStep(upgradeSteps.NewRegisterDoguVersionStep),
			initfx.AsInstallOrChangeStep(upgradeSteps.NewUpdateDeploymentStep),
			initfx.AsInstallOrChangeStep(upgradeSteps.NewDeleteExecPodStep),
			initfx.AsInstallOrChangeStep(upgradeSteps.NewRevertStartupProbeStep),
			initfx.AsInstallOrChangeStep(upgradeSteps.NewDeleteDevelopmentDoguMapStep),
			initfx.AsInstallOrChangeStep(upgradeSteps.NewInstalledVersionStep),
			initfx.AsInstallOrChangeStep(upgradeSteps.NewDeploymentUpdaterStep),

			// use-cases
			fx.Annotate(usecase.NewDoguDeleteUseCase, fx.ParamTags(`group:"deleteSteps"`)),
			fx.Annotate(usecase.NewDoguInstallOrChangeUseCase, fx.ParamTags(`group:"installOrChangeSteps"`)),

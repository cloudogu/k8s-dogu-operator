...
func buildTCPEntries(serviceName string, exposedPorts []serviceaccess.ExposedPort) []expv1.TCPEntry {
	...
	for _, exposedPort := range exposedPorts {
		if normalizedProtocol(exposedPort) != protocolTCP {
			continue
		}
...
func buildUDPEntries(serviceName string, exposedPorts []serviceaccess.ExposedPort) []expv1.UDPEntry {
	...
	for _, exposedPort := range exposedPorts {
		if normalizedProtocol(exposedPort) != protocolUDP {
			continue
		}
...
func normalizedProtocol(exposedPort serviceaccess.ExposedPort) string {
	if exposedPort.Protocol == "" {
		return protocolTCP
	}

	return strings.ToLower(exposedPort.Protocol)
}

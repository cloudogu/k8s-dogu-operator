...
func TestNewRemoveSensitiveDoguConfigStep(t *testing.T) {
	t.Run("Successfully created step", func(t *testing.T) {
		step := NewRemoveSensitiveDoguConfigStep(newMockDoguConfigRepository(t))

		assert.NotNil(t, step)
	})
}

func TestRemoveSensitiveDoguConfigStep_Run(t *testing.T) {
	tests := []struct {
		name                   string
	...
	}{
		{
			...
			want: steps.StepResult{
				Err: fmt.Errorf("could not delete snesitive dogu config: %w", assert.AnError),
			},
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			rdc := &RemoveSensitiveDoguConfigStep{
				sensitiveDoguRepository: tt.doguConfigRepositoryFn(t),
			}

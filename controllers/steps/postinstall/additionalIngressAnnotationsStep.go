...
func (aias *AdditionalIngressAnnotationsStep) Run(ctx context.Context, doguResource *v2.Dogu) steps.StepResult {
	...
	ingressAnnotationsChanged, err := aias.checkForAdditionalIngressAnnotations(doguService, doguResource)
	if err != nil {
		return steps.RequeueWithError(err)
	}

	if ingressAnnotationsChanged {
		err = aias.setDoguAdditionalIngressAnnotations(ctx, doguService, doguResource)
		if err != nil {
			return steps.RequeueWithError(err)
		}
	}
...
func (aias *AdditionalIngressAnnotationsStep) checkForAdditionalIngressAnnotations(doguService *v1.Service, doguResource *v2.Dogu) (bool, error) {
...
func (aias *AdditionalIngressAnnotationsStep) setDoguAdditionalIngressAnnotations(ctx context.Context, doguService *v1.Service, doguResource *v2.Dogu) error {
	err := aias.annotator.AppendIngressAnnotationsToService(doguService, doguResource.Spec.AdditionalIngressAnnotations)
	if err != nil {

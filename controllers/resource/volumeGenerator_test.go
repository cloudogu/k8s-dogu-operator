...
func Test_createVolumeMounts(t *testing.T) {
	t.Run("should create static volumeMounts", func(t *testing.T) {
		...
		assert.Equal(t, doguHealth, volumeMounts[2].Name)
		assert.True(t, volumeMounts[2].ReadOnly)
		assert.Equal(t, "/etc/ces/health", volumeMounts[2].MountPath)

		assert.Equal(t, ldapDoguResource.GetEphemeralDataVolumeName(), volumeMounts[3].Name)
		assert.False(t, volumeMounts[3].ReadOnly)
		assert.Equal(t, "/var/ces/state", volumeMounts[3].MountPath)
		assert.Equal(t, "state", volumeMounts[3].SubPath)

		assert.Equal(t, globalConfig, volumeMounts[4].Name)
		assert.True(t, volumeMounts[4].ReadOnly)
		assert.Equal(t, "/etc/ces/config/global", volumeMounts[4].MountPath)
	...
	t.Run("should create own dogu.json volume mount", func(t *testing.T) {
		...
		assert.Equal(t, "ldap-dogu-json", volumeMounts[7].Name)
		assert.True(t, volumeMounts[7].ReadOnly)
		assert.Equal(t, "/etc/ces/dogu_json/ldap", volumeMounts[7].MountPath)
	})
	...
	t.Run("should create dogu volumeMounts", func(t *testing.T) {
		...
		// then
		assert.Len(t, volumeMounts, 11)

		assert.Equal(t, "ldap-dogu-json", volumeMounts[7].Name)
		assert.True(t, volumeMounts[7].ReadOnly)
		assert.Equal(t, "/etc/ces/dogu_json/ldap", volumeMounts[7].MountPath)

		assert.Equal(t, ldapDoguResource.GetDataVolumeName(), volumeMounts[8].Name)
		assert.False(t, volumeMounts[8].ReadOnly)
		assert.Equal(t, volumes[0].Path, volumeMounts[8].MountPath)
		assert.Equal(t, volumes[0].Name, volumeMounts[8].SubPath)

		assert.Equal(t, ldapDoguResource.GetEphemeralDataVolumeName(), volumeMounts[9].Name)
		assert.False(t, volumeMounts[9].ReadOnly)
		assert.Equal(t, volumes[1].Path, volumeMounts[9].MountPath)
		assert.Equal(t, volumes[1].Name, volumeMounts[9].SubPath)

		assert.Equal(t, volumes[2].Name, volumeMounts[10].Name)
		assert.False(t, volumeMounts[10].ReadOnly)
		assert.Equal(t, volumes[2].Path, volumeMounts[10].MountPath)
	})
}
...
func Test_createVolumes(t *testing.T) {
	...
	t.Run("should create own dogu.json volume", func(t *testing.T) {
		...
		assert.Equal(t, "dogu-spec-ldap", volumes[7].VolumeSource.ConfigMap.LocalObjectReference.Name)
	...
	t.Run("should create dogu volumes", func(t *testing.T) {
		...
		// then
		require.NoError(t, err)
		assert.Len(t, volumes, 9)

		assert.Equal(t, nodeMasterFile, volumes[0].Name)
		assert.Equal(t, ldapDoguResource.GetPrivateKeySecretName(), volumes[1].Name)
		assert.Equal(t, "dogu-health", volumes[2].Name)
		assert.Equal(t, ldapDoguResource.GetEphemeralDataVolumeName(), volumes[3].Name)
		assert.Equal(t, globalConfig, volumes[4].Name)
		assert.Equal(t, normalConfig, volumes[5].Name)
		assert.Equal(t, sensitiveConfig, volumes[6].Name)
		assert.Equal(t, "ldap-dogu-json", volumes[7].Name)

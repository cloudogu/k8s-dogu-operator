...
func TestEnsureWarpMenuEntry(t *testing.T) {
	...
	t.Run("should  give error for empty dogu resource", func(t *testing.T) {
	...
	t.Run("Warp tag exists usecase", func(t *testing.T) {
		...
		t.Run("should do nothing to warp menu entries if it exists, and has same value", func(t *testing.T) {
			fetcher := newMockLocalDoguFetcher(t)
			client := newMockWarpmenuentryClient(t)
			fetcher.EXPECT().FetchForResource(ctx, doguResource).Return(doguDescriptor, nil)
		...
		t.Run("errorcase: should create warp menu entries if it exists, but has a different value, error while updating the warp menu entry", func(t *testing.T) {
			fetcher := newMockLocalDoguFetcher(t)
			client := newMockWarpmenuentryClient(t)
			fetcher.EXPECT().FetchForResource(ctx, doguResource).Return(doguDescriptor, nil)
	...
	t.Run("Warp tag does not exist usecase", func(t *testing.T) {
		...
		t.Run("errorcase: should return error if there is an error fetching warp menu entries if warp tag does not exist ", func(t *testing.T) {
			fetcher := newMockLocalDoguFetcher(t)
			client := newMockWarpmenuentryClient(t)

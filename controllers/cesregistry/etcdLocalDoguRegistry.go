...
func (er *EtcdLocalDoguRegistry) UnregisterAllVersions(_ context.Context, name QualifiedDoguName) error {
	err := er.registry.DoguConfig(name.SimpleName).RemoveAll()
	if err != nil && !registry.IsKeyNotFoundError(err) {
		return fmt.Errorf("failed to remove dogu config: %w", err)
	}

	err = er.etcdRegistry.Unregister(name.SimpleName)
	if err != nil && !registry.IsKeyNotFoundError(err) {
		return fmt.Errorf("failed to unregister dogu %s: %w", name, err)
	}
...
func (er *EtcdLocalDoguRegistry) Reregister(_ context.Context, _ QualifiedDoguName, _ *core.Dogu) error {
...
func (er *EtcdLocalDoguRegistry) GetCurrent(_ context.Context, name QualifiedDoguName) (*core.Dogu, error) {
	return er.etcdRegistry.Get(name.SimpleName)
}
...
func (er *EtcdLocalDoguRegistry) IsEnabled(_ context.Context, name QualifiedDoguName) (bool, error) {
	return er.etcdRegistry.IsEnabled(name.SimpleName)
}

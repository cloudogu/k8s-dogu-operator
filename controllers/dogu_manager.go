...
func NewDoguManager(client client.Client, ecosystemClient ecoSystem.EcoSystemV2Interface, operatorConfig *config.OperatorConfig, eventRecorder record.EventRecorder) (*DoguManager, error) {
	...
	installManager := NewDoguInstallManager(client, mgrSet, eventRecorder, configRepos, clientSet.NetworkingV1().NetworkPolicies(operatorConfig.Namespace), operatorConfig.Namespace, clientSet.NetworkingV1().Ingresses(operatorConfig.Namespace))
	if err != nil {
		return nil, err
	}

...
This guide applies when the CRD has changed and must be released.

> [!IMPORTANT] Release the CRD Helm chart first, the operator Helm chart second
>
> This way, dependency errors between the operator and the CRD can be caught earlier and the operator uses the correct version of the CRD.

> [!IMPORTANT] Major CRD version changes may break the operator.
>
> Double-check whether a major version change is really what you want and also check the version dependency annotation `k8s.cloudogu.com/ces-dependency/k8s-dogu-operator-crd` in `$workspace/k8s/helm/Chart.yaml`

1. change to the `develop` branch and pull in any further changes
2. ensure the new feature or bug fix has arrived in `develop`
3. log in to the desired Helm registry
   - `helm registry login ${target-registry}`
   - keep your account login and passphrase handy
4. find the current CRD component version
   - look in the desired Helm registry
...
7. review and commit any changes to YAML files that might occur during `kubebuilder` usage
   - these should consist only of changes to whitespace or the Kubebuilder version
